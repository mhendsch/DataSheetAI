...
def createTable(db, columns, table_name):
    ...
    if (not isinstance(columns, list) or len(columns) == 0):
        ...
        schema_manager.writeError(error_message="Columns must be a non-empty list. Please provide a valid list of columns.")
        return 1
    if (table_name == ""):
        print("Table name is empty. Please provide a valid table name.")
        schema_manager.writeError(error_message="Table name is empty. Please provide a valid table name.")
        return 1
    if (not isinstance(table_name, str)):
        print("Table name must be a string. Please provide a valid table name.")
        schema_manager.writeError(error_message="Table name must be a string. Please provide a valid table name.")
        return 1
    if (not all(isinstance(col, str) for col in columns)):
        print("All column names must be strings. Please provide a valid list of column names.")
        schema_manager.writeError(error_message="All column names must be strings. Please provide a valid list of column names.")
        return 1
    # Connect to database
...
def insertData(db, df, table_name):
    ...
    if (df.empty):
        ...
        schema_manager.writeError(error_message="DataFrame is empty. No data to insert.")
        return 1
    if (table_name == ""):
        print("Table name is empty. Please provide a valid table name.")
        schema_manager.writeError(error_message="Table name is empty. Please provide a valid table name.")
    ...
    if (not isinstance(df, pd.DataFrame)):
        ...
        schema_manager.writeError(error_message="Input data must be a pandas DataFrame. Please provide a valid DataFrame.")
        return 1
    if (len(df.columns) == 0):
        print("DataFrame has no columns. Please provide a DataFrame with columns.")
        schema_manager.writeError(error_message="DataFrame has no columns. Please provide a DataFrame with columns.")
        return 1
    ...
    if cursor.fetchone()[0] == 0:
        ...
        schema_manager.writeError(error_message=f"Table '{table_name}' does not exist.")
        conn.close()
        return 1
...
def queryData(db, query):
    ...
    if (sql_validator.checkSQL(db, query) == 1):
        ...
        schema_manager.writeError(error_message="Query failed validation. Please provide a valid SQL query.")
        conn.close()
        return 1
